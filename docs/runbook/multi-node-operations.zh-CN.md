...
| Redis 连接断开或被拒                           | `bili_syncplay_events_total{event="redis_connection_error"}`、`redis_connection_error` 日志行 | `component` / `role` 指明是哪条连接，`code` 指明套接字故障；短时故障先查宿主机 `apt` 历史有无自动升级重启 Redis | 恢复 Redis；短暂抖动只需确认计数不再增长             |

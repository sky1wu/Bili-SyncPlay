...
| 过期房间不再被回收（reaper 停摆）              | `events_total{event="room_expired_deleted"}` 不再增长而 `{event="room_expiry_scheduled"}` 持续增长；`{event="room_persist_failed"}`（reason `room_reaper_failed`） | reaper 定时器是否还在跑、Redis 连通性、`ROOM_CLEANUP_INTERVAL_MS` | 恢复 Redis；确认清理周期未被调大。**别看 `rooms_non_expired`**：房间一到 `expiresAt` 就从该 gauge 掉出，与是否真被删无关；也别单看 `rooms_expired_deleted_total`，它含惰性读取路径，reaper 停了仍可能增长 |

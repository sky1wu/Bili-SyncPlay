...
| 空房不再回收，`rooms_non_expired` 只涨不落     | `bili_syncplay_rooms_expired_deleted_total`（房间数）、`room_persist_failed` 日志 | reaper 是否还在扫描、`EMPTY_ROOM_TTL_MS`、Redis 连通性 | 恢复 Redis；确认 `ROOM_CLEANUP_INTERVAL_MS` 未被调大 |

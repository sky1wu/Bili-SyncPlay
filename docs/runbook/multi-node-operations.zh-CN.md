...
| 过期房间不再被回收（reaper 停摆）              | `rate(bili_syncplay_room_reaper_sweeps_total{result="ok"}[<窗口>])` 降到 0；`{result="error"}` 增长则是每轮都在失败。正常值是每秒 `1000/ROOM_CLEANUP_INTERVAL_MS` 次（默认 60000 时为 `1/60`）——`rate()` 的单位是每秒，而配置是毫秒。窗口必须取 `ROOM_CLEANUP_INTERVAL_MS` 的**数倍**，否则健康的 reaper 会落在两次采样之间被读成 0；`[5m]` 只适用于清理周期远小于 1 分钟的情况。**仅 Room Node 有这条序列**——独立 Global Admin 不跑 reaper，根本不导出它 | reaper 定时器是否还在跑、Redis 连通性、`ROOM_CLEANUP_INTERVAL_MS` | 恢复 Redis；确认清理周期未被调大。**这三个都判不了停摆**：`bili_syncplay_rooms_non_expired`（房间一到 `expiresAt` 就掉出该 gauge，与是否真被删无关）、`bili_syncplay_rooms_expired_deleted_total`（含惰性读取路径，reaper 停了仍会涨）、`bili_syncplay_events_total{event="room_expired_deleted"}`（只在某轮真删到东西时才记，持续访问会让惰性路径抢先删光，健康的 reaper 也可以长期不记） |

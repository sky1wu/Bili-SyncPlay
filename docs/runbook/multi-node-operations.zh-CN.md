...
当上面某个写把房间留成「无成员且无过期时间」时，做这件事的节点会**把这笔债记下来**，由 reaper
在之后某一跳偿还——**没有任何扫描**去寻找这类房间。`room_leave_orphan_possible` 与
`room_rollback_failed` 点名的就是欠着一笔债的房间；其中任何一条持续小量出现，说明它背后的那个
写在失败。节点若在下一次清扫之前重启，会忘记自己的债，所以重启前后成批出现这些行时，可能留下
需要手工过期的房间。
...
点名的房间可能已经「无成员且无过期时间」，任何 reaper 都不会回收它。离房本身不再因为这件事失败。

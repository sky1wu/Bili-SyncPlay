...
| Expired rooms stop being reclaimed (reaper stalled)               | `events_total{event="room_expired_deleted"}` flat while `{event="room_expiry_scheduled"}` keeps climbing; `{event="room_persist_failed"}` (reason `room_reaper_failed`) | Whether the reaper timer still runs, Redis connectivity, `ROOM_CLEANUP_INTERVAL_MS` | Restore Redis; confirm the cleanup interval was not raised. **Do not watch `rooms_non_expired`**: a room leaves that gauge the moment `expiresAt` passes, deleted or not. Nor `rooms_expired_deleted_total` alone — it also counts the lazy read path, so it can climb with the reaper dead |

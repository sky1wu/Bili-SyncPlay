...
| Expired rooms stop being reclaimed (reaper stalled)               | `rate(bili_syncplay_room_reaper_sweeps_total{result="ok"}[<window>])` drops to 0; a climbing `{result="error"}` means every pass is failing. Healthy is `1000/ROOM_CLEANUP_INTERVAL_MS` sweeps per second (`1/60` at the 60000 default) — `rate()` is per second, the setting is milliseconds. The window must be **several times** `ROOM_CLEANUP_INTERVAL_MS`, or a healthy reaper falls between samples and reads as 0; `[5m]` only suits intervals well under a minute. Room nodes only — the standalone global admin runs no reaper and does not export this series at all | Whether the reaper timer still runs, Redis connectivity, `ROOM_CLEANUP_INTERVAL_MS` | Restore Redis; confirm the cleanup interval was not raised. **None of these can answer it**: `bili_syncplay_rooms_non_expired` (a room leaves that gauge the moment `expiresAt` passes, deleted or not), `bili_syncplay_rooms_expired_deleted_total` (also fed by the lazy read path, so it climbs with the reaper dead), `bili_syncplay_events_total{event="room_expired_deleted"}` (logged only on a pass that collected something — steady traffic lets the lazy path empty the backlog first, so a healthy reaper can stay silent for hours) |

...
| Empty rooms stop being reclaimed; `rooms_non_expired` only climbs | `bili_syncplay_rooms_expired_deleted_total` (rooms), `room_persist_failed` logs        | Whether the reaper still sweeps, `EMPTY_ROOM_TTL_MS`, Redis connectivity | Restore Redis; confirm `ROOM_CLEANUP_INTERVAL_MS` was not raised               |

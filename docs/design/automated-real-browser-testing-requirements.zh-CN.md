...
| REQ-F044 | `admin.authorization.{viewer,operator,admin}.ui-visibility`、`admin.authorization.viewer.server-rejection`、`admin.authorization.{operator,admin}.server-success`                                                       | `local-visible`：`admin.authorization.{viewer,operator,admin}.ui-visibility`；`server-roundtrip`：`admin.authorization.viewer.server-rejection`、`admin.authorization.{operator,admin}.server-success`                                    |
...
- **REQ-F001**：测试运行器必须从仓库根目录通过一个稳定 npm 脚本启动，脚本负责构建前置产物、启动运行时依赖、运行用例和清理资源。完整矩阵所需 Redis 必须由 coordinator 启动受管本地进程，或通过显式 `REDIS_URL` 接受外部实例后完成健康检查和隔离；不得因 Redis 未供应而跳过 `REQ-F037`。

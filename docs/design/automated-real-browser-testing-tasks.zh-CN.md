...
- **验收**：任一展开键没有或重复匹配策略、catalog 漏键、多键、重复声明键、空 `requiredAssertionKeys`、策略/类别错配，或与第 5.1 节 priority/requirement/policy 映射漂移时检查失败。表驱动判别力测试分别删除 `playback.user.seek`、`background.terminate.recover-popup` 的旧 popup/port 关闭证据或新文档 port 首事件断言、`background.terminate.recover-message` 的实际 content 设置首事件断言或 `peer-result`、`popup.advanced-info.close` 的真实 summary 点击或内容隐藏断言、`popup.server-url.{adjust-default,adjust-trim}` 各自的调整输入、回填值或包含该值的反馈断言、`page-ui.fullscreen-mount` 的进入全屏动作、当前全屏宿主挂载或可见/可操作断言、`page-ui.fullscreen-exit-remount` 的退出全屏动作、旧全屏宿主脱离或普通容器重新可见/可操作断言、`page-ui.settings-popover.{close,quick-disable}`、`recovery.browser-offline-online` 的原成员身份复用/错误清除/恢复后跨端结果、`recovery.server-restart` 的 Redis 状态保留/新鲜房间态/原身份重连/恢复后跨端结果、`recovery.tab-reload` 的新 content 文档/标准化 URL 重新绑定/hydration/重载后跨端结果、`recovery.shared-tab-close` 的绑定清除/房间与共享视频保留/不自动建 tab 稳定窗口/显式重新打开认领/恢复后跨端结果、`admin.navigation.events` 的目标 path、页头标题、侧栏选中项或页面可见内容断言、`admin.page.events.filter.include-system`、`admin.page.rooms.sort.created-at.desc`、`admin.page.rooms.selection.{select,clear}` 的选中数量/批量栏可见性证据、`admin.page.overview.{load,refresh,error-retry,auto-refresh.enable,auto-refresh.disable}` 各自的 readyz 查询断言、`admin.page.rooms.{load,refresh,error-retry,auto-refresh.enable,auto-refresh.disable}` 各自的房间详情查询断言、两个 auto-refresh disable 的 `stability-window`、`admin.governance.{close-room,expire-room,clear-shared-video,kick-member,disconnect-session}.cancel` 或 `admin.governance.batch.{close-room,expire-room}.cancel` 中任一键的确认框关闭/重开状态重置/无请求/房间与客户端稳定证据、`admin.governance.batch.result.close` 的结果框关闭或无重复请求稳定窗口证据、`admin.governance.disconnect-session.success`、`admin.authorization.operator.server-success`、任一必要 `assertionKey`、`playback.user.seek` 的 `peer-result` 和 `stability-window` 证据、`browser.exit.no-session-recovery` 的重启后 `initiator-visible` 证据，把 `ownership.owner-leave` 从 `peer-sync` 误标成 `local-visible`，并尝试用同操作的 `server-result` 补 `peer-result` 缺口；这些对照都必须失败。同需求兄弟操作（包括高级信息 open/close、服务器地址普通保存/默认值调整/裁剪调整/非法值/重连、fullscreen enter/exit、popover open/close/quick-disable、四个互有独立终态的恢复操作、五个管理导航、房间选择/清空选择、不同管理筛选字段、排序字段/方向、auto-refresh enable/disable、五个单项与两个批量治理取消和 operator/admin 的授权成功）、未知键、花括号/通配键、只写 tag/requirement ID、空测试、skip、未执行断言、漏给任一自动加载/刷新、生命周期或 runner 驱动的非手势操作分配执行 ID、把同一 `operationExecutionId` 分配给两个不同的 catalog 操作实例或同一次尝试内重复 `evidenceId` 均不能补缺；`room_not_found` 或任意用户可见错误也不能替 `recovery.server-restart` 的 Redis 保留状态成功路径。另有分组反例把同一次 seek 或 share 的必要断言拆到两个 `operationExecutionId`、两个场景或原始尝试与诊断重试，所有部分证据都通过也必须报告未覆盖；正向聚合测试则让 P0 smoke、分域场景和一次诊断重试各自用独立执行组提交该操作的完整必要证据，coverage 通过并保留三组记录。首次尝试或任一场景失败/skip 时，完整 job 仍失败。
...
- **工作**：owner 打开页面和 popup，点击真实高级信息 summary 打开面板、保存随机 server，再点击同一 summary 关闭并确认内部内容隐藏，然后建房、复制邀请；member 在自己的 popup 完成相同的打开、保存同一个 server 和关闭流程后再输入邀请入房；owner 分享，在双方页面逐一确认媒体从初始解析起静音且 fixture 从未调用 `play()`，再由 owner 经真实用户入口执行 play、seek、rate、pause；member 离房。
...
- **工作**：覆盖高级信息面板 open/close、合法/非法服务器地址、Enter join、确认取消、替换分享、打开共享视频、复制、设置持久化、pending 和服务端错误。高级信息 open/close 分别点击真实 summary，关闭后断言服务器地址、页内按钮设置和日志内容重新隐藏。`save-ws`、`save-wss` 分别在保存后通过 create/join 触发真实握手；`adjust-default` 输入空白并核验后端默认地址回填及反馈，`adjust-trim` 输入带首尾空格的合法地址并核验发送值已裁剪、回填值及反馈；`reconnect-after-change` 在已入房时切换独立端点并验证重连，不用同一连接证据同时代替这些操作键。
- **验收**：使用 E2E-111 的 coverage check 证明需求规格第 5.1 节为 REQ-F010～F017 展开的每个操作键均有本次实际通过的必要断言及类别证据；删除 `popup.advanced-info.{open,close}` 任一键或关闭后的内容隐藏断言、`popup.server-url.{adjust-default,adjust-trim}` 任一路径的输入/回填/反馈断言、`room.join-enter`、任一确认/取消分支或任一错误键时检查失败，不能由同需求下的兄弟操作代替。
- **估算**：3～4 天
...
- **工作**：覆盖 play/pause/seek/永久 rate、waiting/stalled/canplay、soft apply、hard seek、ended 和 video 重建。另在操作者已有活动 rate-only soft apply 会话时执行 `playback.user.hold-fast-forward`：确定性 Bilibili 页面夹具按当前站点语义处理真实 ArrowRight keydown/repeat/keyup，按住期间把 live rate 临时切到 3×，释放时恢复按下前保存的纠偏倍率；用例必须通过浏览器键盘 API 驱动，不得直接赋值 `video.playbackRate` 或 `currentTime`。记录按住前的房间基础倍速、纠偏倍率和两端位置，按住到足以让发起端前进量超过普通基础倍速推进量与动态容差后释放，并持续采集该手势触发的 `playback:update`、服务端结果和两端播放器状态。
- **验收**：每类信号既验证最终结果，也验证没有反向回声污染；固定窗口等待不进入用例。`playback.user.hold-fast-forward` 必须提交独立的 `initiator-visible`、`server-result`、`peer-result` 和 `stability-window` 证据：发起端实际进入临时 3× 并在释放后继续/完成原纠偏；该手势期间每个被服务端接受的快照都携带按住前的房间基础倍速而非 3× 或纠偏倍率，同时至少一个快照的 `currentTime` 体现超出普通播放推进量的前向跳跃；接收端在保持基础倍速的同时于统一上限内跟进该位置，最后双方稳定回到同一基础倍速。用文件备份分别做两个单因素负向对照：临时让 `isRateControlGesture` 把 ArrowRight 当成永久倍速接管时必须只因 3×/纠偏倍率污染房间而失败；临时让 ArrowRight keydown 不刷新 `lastUserGestureAt` 时必须在仍真实进入 3×、且基础倍速未污染的前提下，只因位置跳跃未及时到达接收端而失败。每次还原后重跑均通过；直接写媒体属性、未建立活动纠偏会话、负向对照因夹具未进入 3× 或其他前置条件失败都不算有判别力。
- **估算**：4～6 天
...
- **工作**：覆盖 service worker terminate、offline/online、服务端重启、tab reload/close、显式离房和浏览器完全退出。worker 恢复必须在双客户端已加入同一房间且通过真实 action popup 分享 fixture 视频后，使用 E2E-002 已验证的调试路径执行两个独立终止周期：先关闭分享用 popup 并确认旧文档/port 消失，终止 worker 后创建全新 action popup，由实际首个新 port 连接唤醒；初始查询允许处于 bootstrap 过渡态，最终用后续 port 和服务端收敛验证恢复。再关闭该 popup 并确认文档/port 消失，重新终止 worker 并重载共享视频 tab，由实际首个 content 设置 hydration 消息唤醒，随后等待房间 hydration，并以真实播放操作验证跨端同步。四个 `REQ-F033` 操作各用独立执行组：offline/online 在恢复在线后等待自动重连、原 member ID 和错误清除；server restart 由 coordinator 终止并重启服务进程但复用 E2E-110 提供的同一 run Redis room/runtime store，等待原身份、新鲜房间态和恢复后跨端播放；tab reload 记录旧/新 content 文档 ID，验证新文档重新绑定同一标准化 URL、hydrate 当前房间态并发起重载后的真实播放；shared tab close 从真实 popup 高级信息读取生产诊断日志以证明 source-tab binding 清除，同时验证 popup 仍在原房间且共享视频未变、稳定窗口内 tab 未自动重建，再从真实 popup 点击“打开共享视频”，验证新/复用 tab 被认领并恢复跨端播放。
- **验收**：每个终止周期都记录本周期目标 ID、旧执行上下文失效、目标消失、实际首个生产事件和新 target 创建时刻，且不得复用同一次终止或恢复证据。popup 周期必须证明旧 popup 文档/port 已消失、新文档标识不同且其 port 连接先于状态查询；首次查询的合法过渡态不算恢复失败，后续 port 与服务端必须收敛到已入房。content 周期必须证明 `content:get-page-share-button-settings` 先于 `content:get-room-state`；若生产顺序改变则按实测更新断言，不能复用旧 popup 或跳过先行事件。`background.terminate.recover-popup`、`background.terminate.recover-room` 分别具备发起端和服务端恢复证据；`background.terminate.recover-message` 还必须具备 member 播放器的 `peer-result` 和稳定窗口证据，不能用 owner 本地 UI 或 server result 代替。两个触发动作前都不得由 popup、alarm、自动重试等其他来源预先唤醒。自动重连与显式离房的 token/成员语义可从用户结果区分；`recovery.browser-offline-online` 只允许离线窗口内出现暂时性错误，在线后必须清除并以原成员身份同步；`recovery.server-restart` 必须证明重启前后的进程不同、Redis 状态保留、原成员身份复用、新鲜房间态和恢复后同步，memory store 丢失后的 `room_not_found` 或任意终态错误都不算通过；`recovery.tab-reload` 必须证明 content 文档确已替换、标准化 URL 重新绑定、hydration 和重载后同步；`recovery.shared-tab-close` 必须证明绑定清除、房间/身份/共享视频保留、不自动建 tab 的稳定窗口、显式重新打开/认领和恢复后同步。浏览器完全退出并重启同一隔离 profile 后，popup 明确处于未加入房间状态，服务端已移除旧会话，且稳定窗口内该会话不会重新出现。删除任一旧文档/port 关闭证据、实际首事件、固定生命周期终态、最终 UI、服务端、peer-result 或稳定窗口断言都必须让 coverage check 失败；四个恢复操作之间以及任意错误终态都不能相互补缺。
- **估算**：4～5 天
...
- **依赖**：E2E-302
- **需求**：REQ-F028
- **工作**：验证 toast、分享按钮拖动、popover 打开、经 pointerleave 或 focusout 正常关闭、popover 内快捷停用分享按钮，以及进入全屏后挂载到全屏宿主、退出全屏后重新挂载到普通页面容器；精细坐标仍留给现有组件测试。
- **验收**：浏览器级用例只断言关键可见性、可操作性和设置结果，不建立大面积脆弱截图基线；`page-ui.fullscreen-mount` 必须在真实进入全屏后证明 UI 位于当前全屏宿主且可见、可操作，`page-ui.fullscreen-exit-remount` 必须执行真实退出全屏，证明旧全屏宿主脱离、UI 在普通页面容器重新可见且可操作，两者提交独立 operation evidence 并互不补缺；`page-ui.settings-popover.{open,close,quick-disable}` 分别提交独立 operation evidence，close 必须让浮层在按钮仍挂载时消失，快捷停用还须证明真实 content→background 设置消息已生效，三个兄弟操作不能互相补缺。
- **估算**：2～3 天
...
- **工作**：由 Windows runner 通过 E2E-only WSL launcher 导入当前构建的生产 bootstrap，并利用现有 `ServerBootstrapDependencies.now` 控制服务端逻辑墙钟；Windows 浏览器跑双客户端。每次运行都启动全新的 server 进程、房间和浏览器 profiles，先固定注入 `+D` 直到 `sync:ping` 原始样本与 popup 诊断估计稳定一致，再在 `T >= 2 × CLOCK_SYNC_INTERVAL_MS` 且以毫秒计 `T > D` 的时长内把偏移从 `+D` 线性降到 `0`，其中 `D >= max(5_000ms, 4 × playing 动态位置容差)`。逐个 pong 采集原始 out/in 分量、当前偏移、已发布估计和服务端版本，确认估计滞后当前钟差超过 playing 动态位置容差后，由 owner 在该窗口连续产生 playing 快照并采样两个播放器。不得调整 Windows 或 WSL 系统时钟；注入逻辑必须保证服务端逻辑时间和播放版本单调不减。
- **验收**：测试 oracle 不使用跨钟 timestamp 差；诊断先证明固定偏移下校时已稳定，再证明漂移窗口内“已发布估计减当前原始钟差”的实测滞后超过动态位置容差，且服务端逻辑时间与快照 `serverTime` 从不倒退、actor/seq 持续向前。正确实现须在连续快照期间保持双播放器收敛。用文件备份临时在当前代码中恢复 #210 前的 `estimatedServerNow = localNow + clockOffsetMs`、再减 `playback.serverTime` 补偿后，在全新的进程、房间和 profiles 复现相同校准与漂移；它必须因每个滞后窗口快照都被过度外推而持续超出动态位置容差，固定 `+D`/`-D` 场景中的抵消不算负向失败。还原后重跑通过。若负向对照因 launcher、采样缺失、未形成足量估计滞后、复用旧运行状态或任一服务端版本时间倒退而红，不能算有判别力。
- **估算**：3～4 天

...
- **只传时长，不传时刻。** 时长跨两个不一致的钟是安全的：接收端只是把它加到自己的锚上。时刻则不然，接收端必须拿服务端时刻减本地时刻，量到的是钟差而非流逝时间（见[运行时不变量](./invariants.zh-CN.md)的「播放计时不变量」一节）。
...
新增错误码：加入请求仍得到 v5 之前的终态 `room_not_found`，其他可能逸出该结果的请求路径
统一降级为 `internal_error`。

...
test("navigation controller auto-shares a sharer same-element multi-part advance without a url change", () => {
  const windowHarness = installWindowStub();
  const runtimeState = createContentRuntimeState();
  runtimeState.activeRoomCode = "ROOM01";
  runtimeState.pendingRoomStateHydration = false;
  runtimeState.activeSharedUrl = "https://www.bilibili.com/video/BVcol?p=1";
  runtimeState.activeSharedByMemberId = "member-1";
  runtimeState.localMemberId = "member-1";

  // The page URL never changes — B站 advances to the next part inside the same
  // <video> element — but the page bridge resolves the new part.
  const pageUrl = "https://www.bilibili.com/video/BVcol";
  let resolvedUrl: string | null = "https://www.bilibili.com/video/BVcol?p=1";
  const autoShareRequests: Array<{
    previousSharedUrl: string;
    nextNormalizedPageUrl: string;
  }> = [];
  ...
  const controller = createNavigationController({
    ...
    getResolvedSharedVideoUrl: () => resolvedUrl,
    debugLog: () => {},
    getNow: () => 10_000,
  });
  ...
  try {
    controller.start();
    // The element auto-advances to part 2 with no href change.
    resolvedUrl = "https://www.bilibili.com/video/BVcol?p=2";
    windowHarness.intervals[0]?.();

    assert.deepEqual(autoShareRequests, [
      {
        previousSharedUrl: "https://www.bilibili.com/video/BVcol?p=1",
        nextNormalizedPageUrl: "https://www.bilibili.com/video/BVcol?p=2",
      },
    ]);
    assert.equal(
      runtimeState.explicitNonSharedPlaybackUrl,
      "https://www.bilibili.com/video/BVcol?p=2",
    );
  } finally {
    windowHarness.restore();
  }
...
test("navigation controller does not auto-share a non-sharer same-element part advance", () => {
  const windowHarness = installWindowStub();
  const runtimeState = createContentRuntimeState();
  runtimeState.activeRoomCode = "ROOM01";
  runtimeState.pendingRoomStateHydration = false;
  runtimeState.activeSharedUrl = "https://www.bilibili.com/video/BVcol?p=1";
  // Another member is the sharer.
  ...
  try {
    ...
    // A non-sharer must not advance the room; the playback binding's end-hold
    // keeps them paused instead.
    assert.deepEqual(autoShareRequests, []);
  } finally {
    windowHarness.restore();
  }

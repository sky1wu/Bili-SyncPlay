...
test("playback binding controller keeps hydration pause guard when shared url is unknown", () => {
  ...
  try {
    ...
    dom.video.paused = false;
    dom.listeners.get("play")?.(new Event("play"));
    assert.ok(queuedPause, "the hydration hold queues its pause");

    // Buffer evidence can arrive while the zero-delay pause is waiting. The
    // ownership marker belongs to the actual pause effect, so it must be newer.
    now = 1_150;
    dom.listeners.get("waiting")?.(new Event("waiting"));
    now = 1_160;
    const pauseTask = queuedPause as () => void;
    pauseTask();

    assert.equal(pausedByGuard, 1);
    assert.equal(runtimeState.explicitNonSharedPlaybackUrl, null);
    assert.equal(runtimeState.lastForcedPauseAt, 1_160);
